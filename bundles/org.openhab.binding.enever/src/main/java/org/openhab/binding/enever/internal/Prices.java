...
public class Prices {
    ...
    private Hashtable<LocalDate, Double> gPrices = new Hashtable<>();
    ...
    public Prices(String apiToken) {
        ...
        this.tomorrowGasUrl = "https://enever.nl/api/gasprijs_morgen.php?token=" + apiToken;
    }
    ...
    public Double getGasPrice() {
        ...
        if (!gPrices.containsKey(date)) {
            return Double.MIN_VALUE;
        }

        // Clean up
        gPrices.remove(date.minusDays(2));

        return gPrices.get(date);
    }
    ...
    private boolean retrieveGasPrice(boolean today) throws EneVerException {
        Payload p = null;

        if (today) {
            p = retrievePayload(todayGasUrl);
        } else {
            p = retrievePayload(tomorrowGasUrl);
        }
        ...
        if (p.getStatus()) {
            ...
            gPrices.put(date, p.getPrices().get(0).getPrijs());
        }
